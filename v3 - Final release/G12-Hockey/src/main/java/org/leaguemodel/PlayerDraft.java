    ...
    public void drafting(List<TeamOnScoreboard> teamsList) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            for (int j = 0; j < teamsList.size(); j++) {
                List<IPlayers> teamPlayersList = teamsList.get(j).getTeam().getPlayers();
                teamPlayersList.add(newPlayersList.get(0));
                teamsList.get(j).getTeam().setPlayers(teamPlayersList);
                logger.debug("Player drafted to team:" + teamsList.get(j).getTeam().getTeamName());
                display.displayMessage("Player drafted to team:" + teamsList.get(j).getTeam().getTeamName());
        ...
        for (int j = 0; j < teamsList.size(); j++) {
            ...
            if (teamPlayersList.size() > 30) {
                for (int i = 0; i < teamPlayersList.size(); i++) {
                    if (teamPlayersList.get(i).getPosition().equals("forward")) {
                        forwardCount++;
                        if (forwardCount > 16) {
                            additionalPlayers.add(teamPlayersList.get(i));
                            teamPlayersList.remove(teamPlayersList.get(i));
                        }
                    } else if (teamPlayersList.get(i).getPosition().equals("defense")) {
                        defenseCount++;
                        if (defenseCount > 10) {
                            additionalPlayers.add(teamPlayersList.get(i));
                            teamPlayersList.remove(teamPlayersList.get(i));
                        }
                    } else if (teamPlayersList.get(i).getPosition().equals("goalie")) {
                        goalieCount++;
                        if (goalieCount > 4) {
                            additionalPlayers.add(teamPlayersList.get(i));
                            teamPlayersList.remove(teamPlayersList.get(i));
