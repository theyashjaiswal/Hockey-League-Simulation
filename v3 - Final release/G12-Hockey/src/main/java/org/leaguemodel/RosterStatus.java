            ...
            for (int j = 0; j < league.getConferences().get(i).getDivisions().size(); j++) {
                for (int k = 0; k < league.getConferences().get(i).getDivisions().get(j).getTeams().size(); k++) {
                    ...
                    for (int h = 0; h < playersList.size(); h++) {
                        if (countGoalie < 2) {
                            if (playersList.get(h).getPosition().equalsIgnoreCase(Position.GOALIE.toString())) {
                                countGoalie++;
                                playersList.get(h).setIsActive(true);
                            }
                        }
                        if (countSkaters < 18) {
                            if (playersList.get(h).getPosition().equalsIgnoreCase(Position.FORWARD.toString()) || playersList.get(h).getPosition().equalsIgnoreCase(Position.DEFENSE.toString())) {
                                countSkaters++;
                                playersList.get(h).setIsActive(true);
                            }
                        }
