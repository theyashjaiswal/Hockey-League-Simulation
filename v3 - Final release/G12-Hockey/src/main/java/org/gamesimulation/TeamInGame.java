...
public abstract class TeamInGame {
    ...
    public List<IPlayerInGame> getPlayers() {
        ...
        forwardLines.stream().map(s -> s.getPlayers()).forEach(collection::addAll);
        defenseLines.stream().map(s -> s.getPlayers()).forEach(collection::addAll);
        goalies.stream().map(s -> s.getPlayers()).forEach(collection::addAll);
        return collection;
    }
